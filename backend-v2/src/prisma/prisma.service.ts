...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(configService: ConfigService<AppConfig, true>) {
    ...
    const configuredRetries = Number(configService.get('database.maxRetries', { infer: true }));
    const configuredRetryDelay = Number(
      configService.get('database.retryDelayMs', { infer: true }),
    );

    this.maxRetries = Number.isFinite(configuredRetries) && configuredRetries > 0
      ? configuredRetries
      : 5;
  ...
  private async connectWithRetry() {
    ...
    const retryDelayMs = Number.isFinite(this.retryDelayMs) ? this.retryDelayMs : 2000;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isLastAttempt) {
          this.logger.error('Database connection failed after all retries.', error as Error);
          throw error;
        }
