...
const useResponsiveScale = () => {
  useEffect(() => {
    const updateScale = () => {
      ...
      const prefersMobile = window.innerWidth < 768 || window.innerHeight < 700;

      const dynamicScale = prefersMobile
        ? 1
        : Math.min(widthScale, heightScale);

      const clampedScale = Math.max(0.9, Math.min(1, Number(dynamicScale.toFixed(3))));

      document.documentElement.style.setProperty('--app-scale', clampedScale);
      document.documentElement.dataset.viewport = prefersMobile || clampedScale < 0.98 ? 'mobile' : 'desktop';
    };
...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-layout text-slate-800 dark:text-slate-100 relative overflow-hidden">
      ...
      <div className="app-root-scale-wrapper">
        <div className="app-root-scale flex relative" style={{ color: 'var(--text-primary)' }}>
          ...
          <div className="flex-1 flex flex-col relative z-10 backdrop-blur-sm">
            <DemoBanner />
            <main className="flex-1 w-full py-6 px-3 sm:py-8 sm:px-6 lg:px-10 animate-fade-in">
              <div className="w-full max-w-7xl mx-auto space-y-6 md:space-y-7">
                <div
                  className="rounded-3xl border border-slate-200/70 dark:border-slate-800/80 shadow-[0_18px_60px_-28px_rgba(15,23,42,0.45)] bg-white/90 dark:bg-slate-900/70 backdrop-blur-xl p-4 sm:p-6 lg:p-8"
                  style={{
