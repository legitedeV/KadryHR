...
export function CardSquare({ title, description, actionSlot, children, className }: CardSquareProps) {
  return (
    <section
      className={`border border-surface-300 bg-surface-50 rounded-md shadow-[0_1px_2px_rgba(31,41,55,0.06)] ${
        className ?? ""
      }`}
    >
      {(title || description || actionSlot) && (
        <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-surface-300">
          <div>
