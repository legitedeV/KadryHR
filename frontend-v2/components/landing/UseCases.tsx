    ...
    <section className="px-6 py-20" id="dla-kogo">
      <div className="mx-auto max-w-6xl space-y-12">
        <div className="space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600">Dla kogo</p>
            ...
            <button
              ...
              className={`rounded-full px-5 py-2 text-sm font-semibold transition ${
                active.id === item.id
                  ...
                  : "border border-surface-200/60 bg-white/70 text-surface-600 hover:border-brand-200 dark:border-surface-700/60 dark:bg-surface-900/60 dark:text-surface-300"
              }`}
            >
              {item.label}
            </button>
        ...
        <div className="grid gap-6 md:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="relative overflow-hidden rounded-3xl border border-brand-200/60 bg-brand-50/60 p-6 dark:border-brand-800/50 dark:bg-brand-950/40">
            ...
            <div className="relative space-y-4">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600">Mini animacja</p>
              <div className="grid gap-3">
                {["Zmiana poranna", "Zmiana popołudniowa", "Zmiana weekend"].map((item, index) => (
                  <div
                    key={item}
                    className={`rounded-2xl border border-white/70 bg-white/80 p-4 text-sm font-semibold text-surface-700 shadow-sm dark:border-white/10 dark:bg-surface-900/70 dark:text-surface-100 ${
