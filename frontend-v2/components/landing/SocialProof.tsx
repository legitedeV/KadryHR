...
export function SocialProof({ content }: { content?: SocialProofContent }) {
  ...
  return (
    <section className="landing-section border-t border-surface-200/70 px-6 py-24 dark:border-surface-900/80" id="opinie">
      <div className="mx-auto max-w-6xl space-y-12">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <Reveal className="space-y-3" delay={80} distance={18}>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600 dark:text-brand-300">Social proof</p>
            <h2 className="text-3xl font-semibold text-surface-900 dark:text-surface-50">
              {content?.heading ?? "Co mówią zespoły z pilotażu."}
            </h2>
          </Reveal>
          <Reveal className="rounded-full border border-surface-200/70 bg-white/80 px-4 py-2 text-xs font-semibold text-surface-600 dark:border-surface-800/60 dark:bg-surface-900/60 dark:text-surface-300" delay={120} distance={16}>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {quotes.map((item, index) => (
            ...
            >
              ...
              {item.role && (
                <p className="mt-4 text-xs font-semibold uppercase tracking-[0.2em] text-surface-500 dark:text-surface-400">
                  {item.role}
                </p>
              )}
            </Reveal>
        ...
        <div className="grid gap-6 md:grid-cols-[1.1fr_0.9fr]">
          <Reveal className="grid gap-4 rounded-3xl border border-brand-200/70 bg-brand-50/70 p-6 dark:border-brand-800/50 dark:bg-brand-950/40" delay={160} distance={20}>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {metrics.map((metric, index) => (
                <Reveal
                  key={metric.label}
                  delay={200 + index * 60}
                  className="rounded-2xl bg-white/80 p-4 text-center shadow-sm transition-transform duration-300 hover:-translate-y-1 dark:bg-surface-900/70"
                  distance={16}
                >
                  ...
                  <p className="mt-1 text-xs text-surface-600 dark:text-surface-300">{metric.label}</p>
                </Reveal>
              ))}
            </div>
          </Reveal>
          <Reveal className="rounded-3xl border border-surface-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-surface-800/60 dark:bg-surface-900/60" delay={220} distance={20}>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600 dark:text-brand-300">Security badge</p>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {[
                ...
                <span key={badge} className="rounded-full border border-brand-200/70 bg-brand-50/70 px-3 py-1 text-xs font-semibold text-brand-700 dark:border-brand-800/60 dark:bg-brand-950/40 dark:text-brand-200">
                  {badge}
                </span>
              ))}
            </div>
