...
export function ContactSection() {
  return (
    <section className="landing-section border-t border-surface-200/70 px-6 py-24 dark:border-surface-900/80" id="kontakt">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-10 md:grid-cols-[1fr_1.1fr]">
          <Reveal className="space-y-6" delay={80} distance={18}>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600 dark:text-brand-300">Umów demo</p>
            ...
            <div className="space-y-4 rounded-3xl border border-surface-200/70 bg-white/80 p-6 shadow-sm transition-transform duration-300 hover:-translate-y-1 dark:border-surface-800/60 dark:bg-surface-900/60">
              <div>
          ...
          <Reveal className="rounded-3xl border border-surface-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-surface-800/60 dark:bg-surface-900/60" delay={160} distance={20}>
            <Suspense fallback={<div className="text-sm text-surface-500 dark:text-surface-400">Ładujemy formularz…</div>}>
              <ContactForm />
            </Suspense>
          </Reveal>
        </div>
