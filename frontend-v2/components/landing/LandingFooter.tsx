...
export function LandingFooter({ content }: { content?: LandingFooterContent }) {
  ...
  return (
    <footer className="border-t border-surface-200/70 bg-surface-50/80 py-12 dark:border-surface-800/60 dark:bg-surface-950/60">
      <div className="mx-auto grid max-w-6xl gap-8 px-6 md:grid-cols-4">
        <div className="space-y-4">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-surface-500 dark:text-surface-400">KadryHR</p>
        ...
        {groups.map((group) => (
          <div key={group.label} className="space-y-2 text-sm text-surface-600 dark:text-surface-300">
            <p className="font-semibold text-surface-900 dark:text-surface-100">{group.label}</p>
            {group.links.map((link) => (
              <Link key={link.label} href={link.href} className="block hover:text-surface-900 dark:hover:text-surface-50">
                {link.label}
              </Link>
            ))}
          </div>
        ...
        <div className="space-y-2 text-sm text-surface-600 dark:text-surface-300">
          <p className="font-semibold text-surface-900 dark:text-surface-100">Legal</p>
          {legalLinks.map((link) => (
            <Link key={link.label} href={link.href} className="block hover:text-surface-900 dark:hover:text-surface-50">
              {link.label}
            </Link>
      ...
      <div className="mx-auto mt-10 flex max-w-6xl flex-col gap-2 px-6 text-xs text-surface-500 md:flex-row md:items-center md:justify-between dark:text-surface-400">
