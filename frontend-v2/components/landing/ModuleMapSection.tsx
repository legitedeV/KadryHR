...
export function ModuleMapSection() {
  return (
    <section id="moduly" className="relative overflow-hidden py-16">
      ...
      <div className="relative mx-auto max-w-5xl px-5">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="relative lg:min-h-[32rem]">
            <div className="relative mx-auto flex max-w-lg flex-col items-center justify-center gap-5 rounded-2xl border border-emerald-400 bg-gradient-to-br from-emerald-50 via-white/[0.04] to-transparent p-8 text-center">
              <span className="text-xs uppercase tracking-[0.4em] text-emerald-700">
                KadryHR — rdzeń
              </span>
              <h3 className="text-xl font-semibold text-surface-900">Panel operacyjny</h3>
            ...
            <div className="mt-6 grid gap-3 lg:mt-0">
              {modules.map((module) => (
                <div
                  key={module.name}
                  className={`group rounded-2xl border border-surface-300 bg-white/80 p-4 text-sm text-surface-600 transition hover:-translate-y-1 hover:border-emerald-300/50 hover:bg-surface-50 ${module.position}`}
                >
