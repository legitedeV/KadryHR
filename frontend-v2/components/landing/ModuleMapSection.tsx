        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6 text-sm text-white/70">
              <p className="text-white/80">Wspólne KPI</p>
          ...
          <div className="relative lg:min-h-[36rem]">
            <div className="relative mx-auto flex max-w-lg flex-col items-center justify-center gap-6 rounded-[2.5rem] border border-emerald-300/30 bg-gradient-to-br from-emerald-400/10 via-white/[0.04] to-transparent p-10 text-center">
              ...
              <h3 className="text-2xl font-semibold text-white">Panel operacyjny</h3>
              <p className="text-sm text-white/70">
                Wszystkie decyzje kadrowe powiązane w jednej osi czasu i jednym
                zestawie danych.
              </p>
              <div className="grid w-full gap-3 text-left text-xs text-white/70">
            ...
            <div className="mt-8 grid gap-4 lg:mt-0">
              {modules.map((module) => (
                <div
                  key={module.name}
                  className={`group rounded-3xl border border-white/10 bg-[#111b18]/80 p-5 text-sm text-white/70 transition hover:-translate-y-1 hover:border-emerald-300/50 hover:bg-[#13201c] ${module.position}`}
                >
                  ...
                  <div className="mt-4 flex items-center gap-3 text-xs">
                    ...
                    <span className="text-white/60">{module.detail}</span>
                  </div>
                </div>
              ))}
            </div>
