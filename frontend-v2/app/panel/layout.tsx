      ...
      <aside className="hidden md:flex md:flex-col w-60 border-r border-slate-200/70 bg-white/80 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
        ...
        <nav className="flex-1 py-4 px-3 text-sm space-y-1">
          ...
          {allowedNav.map((item) => {
            ...
            const showBadge = item.href === "/panel/powiadomienia" && unreadCount > 0;
            return (
              <Link
                key={item.href}
                href={item.href}
              ...
              >
                ...
                <div className="ml-auto flex items-center gap-2">
                  {showBadge && (
                    <span className="inline-flex h-5 min-w-[1.5rem] items-center justify-center rounded-full bg-brand-600 px-2 text-[11px] font-semibold text-white">
                      {unreadCount}
                    </span>
                  )}
                  {item.permissions && (
                    <span className="text-[10px] font-semibold uppercase tracking-wide text-brand-600 dark:text-brand-300">
                      Wymaga uprawnień
                    </span>
                  )}
                </div>
