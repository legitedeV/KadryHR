...
export type LogoProposalStatus =
  | "DRAFT"
  | "SUBMITTED"
  | "APPROVED"
  | "REJECTED"
...
export type LogoProposalVote = "APPROVE" | "NEUTRAL" | "REJECT";

export interface LogoProposalFeedback {
  id: string;
  userId: string;
...
export interface LogoProposal {
  ...
  feedbacks?: LogoProposalFeedback[];
}

export interface LogoProposalListResponse {
  data: LogoProposal[];
  total: number;
  take: number;
  skip: number;
...
// Logo proposals API
export interface LogoProposalListQuery {
  status?: LogoProposalStatus;
  skip?: number;
  take?: number;
}

export interface CreateLogoProposalPayload {
  title: string;
  description?: string;
  primaryColor: string;
...
export interface UpdateLogoProposalPayload {
  title?: string;
  description?: string;
  primaryColor?: string;
  secondaryColor?: string;
...
export interface FeedbackLogoProposalPayload {
  vote: LogoProposalVote;
  comment?: string;
}

export interface ApproveLogoProposalPayload {
  applyToOrganisation?: boolean;
}

export interface RejectLogoProposalPayload {
  reason?: string;
}

const LOGO_PROPOSALS_PREFIX = "/logo-proposals";

export async function apiListLogoProposals(
  query: LogoProposalListQuery = {},
): Promise<LogoProposalListResponse> {
  apiClient.hydrateFromStorage();
  const search = new URLSearchParams();
  ...
  return apiClient.request<LogoProposalListResponse>(
    `${LOGO_PROPOSALS_PREFIX}${queryString ? `?${queryString}` : ""}`,
  );
}

export async function apiGetLogoProposal(id: string): Promise<LogoProposal> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposal>(`${LOGO_PROPOSALS_PREFIX}/${id}`);
}

export async function apiCreateLogoProposal(
  payload: CreateLogoProposalPayload,
): Promise<LogoProposal> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposal>(LOGO_PROPOSALS_PREFIX, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}

export async function apiUpdateLogoProposal(
  id: string,
  payload: UpdateLogoProposalPayload,
): Promise<LogoProposal> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposal>(`${LOGO_PROPOSALS_PREFIX}/${id}`, {
    method: "PATCH",
...
export async function apiSubmitLogoProposal(id: string): Promise<LogoProposal> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposal>(`${LOGO_PROPOSALS_PREFIX}/${id}/submit`, {
    method: "POST",
  });
}

export async function apiSendLogoProposalFeedback(
  id: string,
  payload: FeedbackLogoProposalPayload,
): Promise<LogoProposalFeedback> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposalFeedback>(`${LOGO_PROPOSALS_PREFIX}/${id}/feedback`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}

export async function apiApproveLogoProposal(
  id: string,
  payload: ApproveLogoProposalPayload,
): Promise<LogoProposal> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposal>(`${LOGO_PROPOSALS_PREFIX}/${id}/approve`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}

export async function apiRejectLogoProposal(
  id: string,
  payload: RejectLogoProposalPayload,
): Promise<LogoProposal> {
  apiClient.hydrateFromStorage();
  return apiClient.request<LogoProposal>(`${LOGO_PROPOSALS_PREFIX}/${id}/reject`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
