...
Legend: **CURRENT** = istnieje w repo i jest używane; **GAP** = brak lub istotny brak pokrycia; **IN_PROGRESS** = rozwijane w bieżącym sprincie; **DONE** = spełnia akceptację i ma testy/regresję. Statusy odnoszą się do rzeczywistego kodu (backend-v2 + frontend-v2).
...
| Time & Attendance (RCP/clock events) | GAP | Brak modeli czasu pracy/zdarzeń, brak kontrolera (tylko grafiki/wnioski). | Brak UI do rejestracji wejść/wyjść. | DONE gdy istnieją modele zdarzeń, start/stop/break endpoints z geofence/device rules i audit log + UI widżet do odbijania. |
...
| Employee mobile / PWA | GAP | API w pełni web; brak offline/push. | Panel responsywny, ale brak manifestu/service worker i widżetu RCP. | DONE gdy panel jest instalowalny (PWA), działa offline retry i ma mobilny clock/schedule/leave flow. |
...
1. Aktualizuj status w tabeli po każdej zmianie modeli Prisma, endpointów lub routów UI — **CURRENT** tylko, gdy działa pełny flow i ma regresję/testy manualne; **DONE** po spełnieniu kryteriów akceptacji.
2. Po dodaniu/zmianie kontrolera lub strony dopisz go w odpowiedniej sekcji inventory (backend/frontend) i dopisz model, jeśli doszedł w `prisma/schema.prisma`.
