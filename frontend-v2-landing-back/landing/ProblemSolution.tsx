    ...
    <section className="px-6 py-16" id="funkcje">
      <div className="mx-auto max-w-6xl space-y-12">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Reveal className="space-y-4 rounded-3xl border border-surface-800/60 bg-surface-900/60 p-6 shadow-sm backdrop-blur" delay={120}>
            <p className="text-sm font-semibold text-surface-400">Co boli dziś</p>
            <div className="grid gap-4">
              {problems.map((item, index) => (
                <Reveal key={item.title} delay={180 + index * 60} className="rounded-2xl border border-surface-700/60 bg-surface-800/70 p-4">
          ...
          <Reveal className="space-y-4 rounded-3xl border border-brand-800/50 bg-brand-950/40 p-6 shadow-sm" delay={180}>
            <p className="text-sm font-semibold text-brand-200">Co daje KadryHR</p>
            <div className="grid gap-4">
              {solutions.map((item, index) => (
                <Reveal key={item.title} delay={200 + index * 60} className="rounded-2xl border border-brand-800/50 bg-surface-900/60 p-4 shadow-sm">
                  <p className="font-semibold text-surface-50">{item.title}</p>
                  <p className="mt-2 text-sm text-surface-300">{item.desc}</p>
                </Reveal>
              ))}
